# 提交检查表

- [x] Alpha来源、确定性规则、策略代码。
- [x] 60天训练与30天历史留出记录，明确历史验证的局限。
- [x] 收益、Sharpe/Sortino、回撤、胜率、换手、滚动30天指标与成本。
- [x] 训练/留出/基准及高成本报告，所有参数尝试保留。
- [x] 表单六段说明草稿和大模型作用说明。
- [x] 可本地运行的静态证据Demo、数据获取与复现脚本。
- [ ] 公开仓库URL、发布并检查公网Demo。
- [ ] 发布介绍项目的X帖：#BitgetHackathon、@Bitget_AI，并转发指定官方帖。
- [ ] 在正式Google Form填写完整说明、材料和X链接。
- [ ] 团队/UID/学校（如适用）/Demo Day等表单信息。

策略稳定性未通过不会自动变为已通过；回测材料完整不等于有获奖竞争力。截止2026-09-27（UTC+8），具体小时未明确，建议提前完成。

官方提交入口：https://forms.gle/WxCSbXVEky5Z2oYN8

一支队最多提交两个不同主题的独立项目，分别填写表单。本项目拟选rToken因子策略，队友Weekend Bounce项目README为休市信息定价，两者不是同一子主题；最终以两队实际填报内容核对。
