...
class Settings:
    ...
    confirmation_mode: str = 'fresh_highs'
...
def features(frame, cfg):
    ...
    if cfg.confirmation_bars > 1:
        if cfg.family == 'breakout' and cfg.confirmation_mode == 'hold_level':
            # Confirm against the level that caused the original breakout, not a
            # second fresh high. Every observation is from a completed bucket.
            anchor = prior_high.shift(cfg.confirmation_bars-1)
            confirmed = enter.shift(cfg.confirmation_bars-1, fill_value=False)
            for lag in range(cfg.confirmation_bars-1):
                confirmed = confirmed & (bars.close.shift(lag) > anchor) & ready.shift(lag, fill_value=False)
            enter = confirmed
        else:
            enter = enter.rolling(cfg.confirmation_bars, min_periods=cfg.confirmation_bars).sum().eq(cfg.confirmation_bars)
